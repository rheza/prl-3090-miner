...
| 6 | Performance optimization | 🟧 in progress | **Tensor cores proven + first optimization done**: [`cuda/src/mma_gemm_sm86.cu`](cuda/src/mma_gemm_sm86.cu) has two Ampere `mma.sync.m16n8k32.s8` int8 GEMMs, both validated bit-for-bit vs golden `C` on the GPU (`cuda/tests/validate_mma.py`). Measured at 1024³: simple one-warp/tile **6.6 TOPS** → **SMEM-tiled 16.7 TOPS (2.5×)**, vs ~284 TOPS GA102 int8 peak. Remaining: `cp.async` double-buffering + bank-conflict-free SMEM + larger warp tiles, then **fuse the noised+transcript path** onto tensor cores, then Nsight tuning. Targets ≥50 TH/s. |
