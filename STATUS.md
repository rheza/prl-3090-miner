...
| 6 | Performance optimization | 🟧 in progress | **Three Ampere `mma.sync.m16n8k32.s8` int8 GEMMs, all golden-verified on the GPU** ([`cuda/src/mma_gemm_sm86.cu`](cuda/src/mma_gemm_sm86.cu), `cuda/tests/validate_mma.py`): simple one-warp/tile → SMEM-tiled 64×64 → **cp.async double-buffered 128×128 = ~54 TOPS at 4096³ (~19% of the ~284 TOPS GA102 int8 peak)**. Trajectory: naive int cores ~0.006 → 6.6 → 16.7 → 53.9 TOPS. Remaining for full speed: bank-conflict-free SMEM (swizzle) + `ldmatrix` + register double-buffering to push toward peak, then **fuse the noised+transcript path** onto this GEMM (the actual miner hot loop), then Nsight tuning. |
